...
bool JSCell::isFunction()
{
	int type = (int)(structure()->typeInfo().type());
	return !strcmp(TypeNames[type], "JSFunction");
}

void JSCell::PrintSelf(ExecState* exec)
{
	int type = (int)(structure()->typeInfo().type());
	dataLogF("{\"type\":\"");
	dataLogF(TypeNames[type]);
	dataLogF("\",\"value\":");
	if (!strcmp(TypeNames[type], "JSFunction"))
	{
		dataLogF("{\"name\":\"");
		JSFunction* func = static_cast<JSFunction*>(this);
		String name = func->calculatedDisplayName(exec);
		dataLogWin(name);
		dataLogF("\",");
		dataLogF("\"content\":\"");
		const SourceCode* sourceCode = func->sourceCode();
		if (sourceCode != NULL)
		{
			String code = sourceCode->toString();
			dataLogWin(code.jsonize());
		}
		else
			dataLogF("Host Function");
		dataLogF("\"}");
	}
	else if (!strcmp(TypeNames[type], "Proxy"))
	{
		dataLogF("{\"target\":");
		JSProxy* proxy = static_cast<JSProxy*>(this);
		proxy->target()->PrintSelf(exec);
		dataLogF("}");
	}
	else if (!strcmp(TypeNames[type], "String"))
	{
		dataLogF("\"");
		dataLogWin(getString(exec).jsonize());
		dataLogF("\"");
	}
	else
	{
...
uint16_t JSCell::getHashCode() const
{
	return static_cast<uint16_t>(reinterpret_cast<uint64_t>(this) % 0x10000);
}
